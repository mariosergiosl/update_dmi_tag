...
- `_service` (na raiz do projeto): servico do OBS (`tar_scm`) que
  ...
  `<revision>` (atualizada a cada release; hoje aponta para `v2.2.9`).
...
git archive --format=tar.gz --prefix=update_dmi_tag-2.2.9/ \
  -o /tmp/update_dmi_tag-2.2.9.tar.gz v2.2.9

# Build com osc (requer osc instalado e configurado) ou rpmbuild direto:
rpmbuild -ta /tmp/update_dmi_tag-2.2.9.tar.gz
```
