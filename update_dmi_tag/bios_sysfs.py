...
# COMPANY: SUSE
# VERSION: 2.2.9
# REVISION: 2026-07-22 - v2.2.9 - executa_amibios_remoto passa a rastrear
#                        se foi esta execucao que instalou o KMP/pacote
#                        userspace (pacote_instalado_pelo_script). Se sim,
#                        desinstala ambos ao final (desinstala_modulo_
#                        remoto, environment.py), independente do
#                        resultado da gravacao, para o alvo nunca ficar
#                        com estado que nao existia antes desta execucao.
#                        Se o KMP correto ja estava presente desde o
#                        inicio (confirma_kmp_kernel_correto_remoto), o
#                        pacote e preservado, nao removido.
# REVISION: 2026-07-22 - v2.2.9 - antes de aceitar "pacote ja instalado"
#                        (rpm -q so confere o NOME do pacote), confere via
#                        confirma_kmp_kernel_correto_remoto (environment.py)
#                        se a %{VERSION} instalada bate com o kernel real
#                        do host; se nao bater, desinstala o pacote errado
#                        e trata como ausente, para o fluxo normal
#                        reinstalar o KMP certo. BUG REAL de campo (host
#                        10.124.60.133): um KMP compilado para o kernel
#                        errado (bug no build, ja corrigido) ficou
#                        instalado apos um fallback bem-sucedido; sem essa
#                        checagem, execucoes seguintes viam "ja instalado"
#                        para sempre e so tentavam modprobe direto nele,
#                        falhando com "Module not found" indefinidamente.
...
#                        10.24.80.96, PERTOSA GA-H81M-S2PH):
...
from .environment import (
    ...
    confirma_kmp_kernel_correto_remoto, desinstala_modulo_remoto,
)
...
def executa_amibios_remoto(ip, ssh_user, sudo_cmd, tag, sysfs_target,
    ...
    modulo_carregado_pelo_script = False
    pacote_instalado_pelo_script = False
    ...
    try:
        ...
        if not iface_pronta:
            ...
            if not modulo_presente:
                ...
                if pacote_ja_instalado:
                    # BUG REAL de campo (2026-07-22): 'rpm -q' so confere o
                    # NOME do pacote. Se um KMP do kernel ERRADO ficou
                    # instalado numa execucao anterior (build errado, ou
                    # residuo de um fallback que instalou sem checar
                    # dependencias), o codigo achava "ja instalado" para
                    # sempre e so tentava modprobe direto nele, falhando
                    # com "Module not found" indefinidamente. Confere a
                    # versao instalada contra o kernel real do host; se
                    # nao bater, desinstala o pacote errado agora (antes de
                    # tentar qualquer outra coisa) e trata como "ausente"
                    # para o fluxo normal reinstalar o KMP certo abaixo.
                    pacote_ja_instalado = confirma_kmp_kernel_correto_remoto(
                        ip, ssh_user, sudo_cmd, module_package,
                        caminho_log, verbose, suprime_tela, caminho_log_local)
                ...
                if not pacote_ja_instalado:
                    if module_rpm_dir:
                        ...
                        pacote_ja_instalado, detalhe_instalacao = instala_modulo_remoto(
                            ...
                            caminho_log, verbose, suprime_tela, caminho_log_local)
                        if pacote_ja_instalado:
                            pacote_instalado_pelo_script = True
                    else:
                        _log("WARNING",
        ...
        # Desinstala o pacote KMP/userspace se foi esta execucao que os
        # instalou (o KMP nao estava correto no alvo antes de comecarmos).
        # Se o KMP correto ja estava presente desde o inicio (confirmado
        # por confirma_kmp_kernel_correto_remoto), pacote_instalado_pelo_
        # script permanece False e o pacote existente e preservado. O
        # objetivo e o alvo nunca acumular estado que nao existia antes
        # desta execucao.
        if pacote_instalado_pelo_script:
            desinstala_modulo_remoto(
                ip, ssh_user, sudo_cmd, module_package,
                DEFAULT_MODULE_USERSPACE_PACKAGE,
                caminho_log, verbose, suprime_tela, caminho_log_local)
